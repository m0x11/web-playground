...
//
// Grading (exposure … invert) is built by media/grading.js; each instance
// owns a tiny inline <svg><filter> for the temperature/tint colour matrix.
// All of it is plain DOM/CSS, so headless export renders it too.
...
import { buildGradingFilter, createBalanceFilter } from '../../media/grading.js';
...
export function mount(el, props, _ctx) {
  ...
  const balance = createBalanceFilter(`gen-media-balance-${++instanceCounter}`);

  el.append(balance.svg, imgEl, videoEl, cycleLayer, placeholder);
  ...
  function gradingFilter(p) {
    return buildGradingFilter(p, balance);
  }
